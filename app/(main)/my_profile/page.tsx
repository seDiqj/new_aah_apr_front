...
const Preloader: React.FC<{ duration?: number }> = ({ duration = 4000 }) => {
  ...
  const [visible, setVisible] = useState(false); // برای fade in/out
  ...
  useEffect(() => {
    ...
    const fadeOut = setTimeout(() => setVisible(false), duration - 400); // 400ms برای transition
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 bg-green-400 opacity-80 mix-blend-screen transition-opacity duration-400"></div>
