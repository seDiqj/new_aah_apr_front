...
const OutcomeForm: React.FC<ComponentProps> = ({mode}) => {

    const {outcomes, setOutcomes, setCurrentTab}: {
        ...
        <>
            <Card className="relative h-full flex flex-col">
                      ...
                      {outcomes.map((item, index) => (
                          ...
                          {item.id != null && 
                              (!readOnly ? <Edit className="cursor-pointer text-orange-500 hover:text-orange-700" onClick={() => {
                                setOutcomeIdForEditOrShow(item.id as unknown as number);
                ...
                <CardFooter className="flex flex-row w-full gap-2 items-start justify-end absolute bottom-5">
                    {cardsBottomButtons(setCurrentTab, "project", undefined, setCurrentTab, "output")}
                </CardFooter>
            </Card>

            {reqForOutcomeEditModel && outcomeIdForEditOrShow && <OutcomeEditModal isOpen={reqForOutcomeEditModel} onOpenChange={setReqForOutcomeEditModel} outcomeId={outcomeIdForEditOrShow} mode={readOnly ? "show" : "edit"} pageIdentifier={mode} ></OutcomeEditModal> }
            {reqForOutcomeShowModel && outcomeIdForEditOrShow && <OutcomeEditModal isOpen={reqForOutcomeShowModel} onOpenChange={setReqForOutcomeShowModel} outcomeId={outcomeIdForEditOrShow} mode={readOnly ? "show" : "edit"} pageIdentifier={mode} ></OutcomeEditModal> }
